...
int main(int argc, char** argv){
    ...
    int scale_bar = 15;
    double scale = 1; //画面との大きさの比
    const int maxscale = 3; //5
    ...
    cv::createTrackbar("scope", "result", &slope, 89, 0);
    cv::createTrackbar("size", "result", &scale_bar, 30, 0);
    ...
    for (;;) {
        ...
        if (scale_bar == 0) {
            ...
            scale = (double)scale_bar / 10;
        }
        
        
        
        if (pre_scale != scale || pre_x_move != x_move || pre_y_move != y_move || pre_slope != slope) {
            ...
            //cv::imshow("test", changing_wall);
            //cv::imwrite("data.jpg",changing_wall);
