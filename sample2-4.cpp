...
void glut_display() {
	...
	GLfloat lightpos0[] = { 5 * cosf((float)g_angle4) * sinf((float)g_angle3),
	...
	glPushMatrix();
	glTranslatef(5 * cosf((float)g_angle4) * sinf((float)g_angle3), 5 * sinf((float)g_angle4),
		5 * cosf((float)g_angle4) * cosf((float)g_angle3));
...
//法線ベクトルをでたらめにすると影がつかなくなる
//日が当たっている所と同じ法線ベクトルにするとおなじくひがあたったようになった。
//法線ベクトルは光源への方向を確定させるためにある

//光源の色は137行目のGLfloat diffuse[] = { 1.0, 1.0, 1.0, 1.0 }で変えられる
//光源色の中から物体色の成分のみを反射し、最終的に描画される色になる。
