...
void glut_display() {
    ...
    glLoadIdentity();
    if (cos(g_angle2) > 0) {
        gluLookAt(g_distance * cos(g_angle2) * sin(g_angle1), g_distance * sin(g_angle2), g_distance * cos(g_angle2) * cos(g_angle1),
...
void glut_idle() {
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, (TEXTURE_WIDTH - frame.cols) / 2,
        (TEXTURE_HEIGHT - frame.rows) / 2, frame.cols, frame.rows, GL_RGB, GL_UNSIGNED_BYTE, frame.data);
    glutPostRedisplay();
}
void draw_pyramid() {
...
void set_texture() {
    const char* inputFileNames[3] = { "hanabi.jpg","itukusima.jpg","moon.jpg" };
    for (int i = 0; i < 3; ++i) {
        ...
        glTexSubImage2D(GL_TEXTURE_2D, 0, (TEXTURE_WIDTH - input.cols) / 2,
            (TEXTURE_HEIGHT - input.rows) / 2, input.cols, input.rows, GL_RGB, GL_UNSIGNED_BYTE, input.data);
    }
}
