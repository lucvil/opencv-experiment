...
void myMouseEventHandler(int event, int x, int y, int flags, void*) {
    ...
    if (event == CV_EVENT_LBUTTONUP) {
        ...
        cv::Mat whiteLined_image_copy = whiteLined_image(roi);
        cv::Mat inpainted_copy = inpainted(roi);
        cv::bitwise_not(whiteLined_image_copy, whiteLined_image_copy);
        cv::bitwise_not(inpainted_copy, inpainted_copy);

        cv::blur(whiteLined_image_copy, whiteLined_image_copy, cv::Size(1, 1));
        cv::blur(inpainted_copy, inpainted_copy, cv::Size(1, 1));
        cv::imshow("image", whiteLined_image);
    }

    //if (event == CV_EVENT_LBUTTONUP) {
        //cv::Rect rec(prevPt,pt);
        //cv::Mat roi(whiteLined_image, rec);
        //bitwise_xor(roi, cv::Scalar::all(255), roi);
        //cv::imshow("image", whiteLined_image);
    //}
}
